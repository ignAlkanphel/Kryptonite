...
public class PreventBeingUsedAbility extends InteractionPrioritizedAbility {

    public static final MapCodec<PreventBeingUsedAbility> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
    ...
    public boolean doesApply(Player other, Entity holder, InteractionHand hand, ItemStack heldStack) {
    ...
    public AbilitySerializer<?> getSerializer() {
        return KryptoniteAbilitySerializers.PREVENT_BEING_USED.get();
    }

    public static class Serializer extends AbilitySerializer<PreventBeingUsedAbility> {

        @Override
        public MapCodec<PreventBeingUsedAbility> codec() {
            return CODEC;
        }

        @Override
        public void addDocumentation(CodecDocumentationBuilder<Ability, PreventBeingUsedAbility> builder, HolderLookup.Provider provider) {
            builder.setName("Prevent Being Used")
                    .setDescription("Prevents other players from being able to \"use\" (right-click) the entity that has this ability and runs actions upon being prevented. In the context of this ability, the \"actor\" is the entity that did the \"usage\" action (right-click) & the \"target\" is the entity that has this ability.")
                    ...
                    .addExampleObject(new PreventBeingUsedAbility(List.of(), List.of(), List.of(), List.of(), List.of(), Optional.empty(), EnumSet.allOf(InteractionHand.class), InteractionResult.SUCCESS, 0, AbilityProperties.BASIC, AbilityStateManager.EMPTY, List.of()))
                    .addExampleObject(new PreventBeingUsedAbility(List.of(new ActorActionBiAction(List.of(new RunCommandAction(new ParsedCommands("title @s actionbar {\"text\": \"Entity cannot be interacted with!\", \"color\": \"red\"}"))))), List.of(), List.of(), List.of(), List.of(), Optional.empty(), EnumSet.allOf(InteractionHand.class), InteractionResult.SUCCESS, 0, AbilityProperties.BASIC, AbilityStateManager.EMPTY, List.of()));
        }
    }
